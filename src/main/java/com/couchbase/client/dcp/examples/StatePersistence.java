...
public class StatePersistence {
  ...
  static String BUCKET = "testbucket";
  static String FILENAME = "state-" + BUCKET + ".json";

  public static void main(String[] args) throws Exception {
    // Connect to localhost and use the testbucket bucket
    ...
    client.controlEventHandler(new ControlEventHandler() {
      ...
      public void onEvent(ChannelFlowController flowController, ByteBuf event) {
        if (DcpSnapshotMarkerRequest.is(event)) {
          System.out.println("Snapshot Request: " + DcpSnapshotMarkerRequest.toString(event));
          flowController.ack(event);
        } else if(RollbackMessage.is(event)) {
        	System.out.println("Control Event: Rollback event");
        	System.out.println(RollbackMessage.seqno(event) + " " + RollbackMessage.toString(event));
        	RollbackMessage.init(event, RollbackMessage.vbucket(event), RollbackMessage.seqno(event));
        }
        event.release();
      }
